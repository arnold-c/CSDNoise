...
#show: article.with(
  ...
  abstract: [
    Early warning signals (EWS) for epidemics are summary statistics of infection time series that show promise in providing advanced warnings of pathogen transmission dynamics approaching the “critical transition”, $R_"E" = 1$ ($R_"E"$ is the effective reproduction number).
    The accuracy of the underlying individual case data is integral to the performance of the EWS, and noise in the data will depend on the dynamics of both the pathogen of interest and other symptomatic presentations (that could result in false positives), the diagnostic test characteristics, and testing rate.
    ...
    We simulated under conditions where measles vaccination rates were falling, such that $R_"E"$ increase from below to above 1 (emergent), and conditions where measles vaccination rates were constant such that $R_"E"$ remains below 1 (null).
