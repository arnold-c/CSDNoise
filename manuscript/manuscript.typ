...
#show: article.with(
  ...
  abstract: [
    Epidemic early warning signals (EWS) are summary statistics of infection time series that show promise in providing advanced warnings of pathogen transmission dynamics approaching the “critical transition”, $R_"E" = 1$ ($R_"E"$ is the effective reproduction number).
    The underlying individual case data accuracy is integral to the performance of the EWS, and noise in the data will depend on the dynamics of both the pathogen of interest and other symptomatic presentations (that could return false positives), the diagnostic test characteristics, and testing rate.
    We simulated test-positive time series of symptomatic cases as the sum of a SEIR measles-like time series and noise, generated by either independent draws from a Poisson distribution or a SEIR rubella-like time series.
    We simulated under conditions where measles vaccination rates were falling, such that $R_"E"$ increase from below to above 1 (emergent), and where measles vaccination rates were constant such that $R_"E"$ remains below 1 (null).
    We demonstrate that proactive outbreak detection with EWS metrics is resilient to decreasing diagnostic accuracy, provided the noise is not dynamical and substantially larger in magnitude than the pathogen of interest.
    In these situations, diagnostic accuracy should not be a precursor to the implementation of an EWS metric-based alert system.
  ],
  word-count: false,
  line-numbers: false,
)
...
This discrete decision must balance the consequences of being overly sensitive i.e., the cost of over-reaction in the absence of a transition, against those relating to being overly specific, i.e., under- or late reactions in the presence of a transition.
...
All simulations and analysis was completed in Julia version 1.11.5 @bezansonJuliaFreshApproach2017.
A pre-print version of this paper has been uploaded to ArXiV @arnoldDiagnosticUncertaintyLimits2025.
